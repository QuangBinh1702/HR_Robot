"""
Convert ArcFace/MobileFaceNet ONNX -> RKNN for RK3588S deployment.

Run this conversion on a host PC (x86_64 Linux) with rknn-toolkit2 installed.

Examples:
    python scripts/rknn/convert_arcface.py
    python scripts/rknn/convert_arcface.py --quantize fp16
    python scripts/rknn/convert_arcface.py --onnx models/arcface_rec.onnx --output models/arcface_rec.rknn
"""
...
def prepare_calibration_dataset(calib_dir: Path, output_file: Path, max_images: int = 200) -> bool:
    ...
    if not calib_dir.exists():
        ...
        print("       Add 100-200 aligned face images (112x112) for better INT8 accuracy.")
        return False
...
def _sanitize_input_shape_for_rknn(input_shape: list) -> list[int] | None:
    ...
    for idx, dim in enumerate(input_shape):
        if isinstance(dim, int):
            ...
            if dim is None or dim_str in ("none", "?", "-1", ""):
...
def convert(onnx_path: Path, output_path: Path, quantize: str = "int8") -> None:
    """Convert ArcFace ONNX model to RKNN."""
    try:
    ...
    if not onnx_path.exists():
        ...
        print("      Tip: run python scripts/download_models.py first")
        sys.exit(1)
    ...
    try:
        ...
        # ArcFace/MobileFaceNet normalization: (x - 127.5) / 127.5
        if input_layout == "nchw":
            ...
            print("[INFO] Using built-in normalization in RKNN config (NCHW)")
        else:
            # Some NHWC exports can be sensitive with RKNN mean/std checks.
            rknn.config(
                target_platform="rk3588",
                optimization_level=0,
            )
            print("[WARN] NHWC input detected: skip mean/std in config.")
            print("[WARN] Apply normalization in runtime preprocess for this model.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert ArcFace/MobileFaceNet ONNX -> RKNN")
    parser.add_argument(
        "--onnx",
        type=str,
        default=str(PROJECT_ROOT / "models" / "arcface_rec.onnx"),
        help="Path to ONNX recognizer model",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=str(PROJECT_ROOT / "models" / "arcface_rec.rknn"),
