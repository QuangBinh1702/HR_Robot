"""
Convert YOLOv8n ONNX -> RKNN for RK3588S deployment.

Recommended flow:
1) Export ONNX from ultralytics with input 640x640.
2) Convert to RKNN on x86 host.
3) Deploy models/yolov8n.rknn to RK3588S board.
"""
...
def convert(onnx_path: Path, output_path: Path, quantize: str = "int8") -> None:
    ...
    dataset_file = PROJECT_ROOT / "data" / "calibration" / "yolov8_dataset.txt"
    ...
    print("--> Configuring YOLOv8n model")
    rknn.config(
        mean_values=[[0, 0, 0]],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert YOLOv8n ONNX -> RKNN")
    parser.add_argument(
        "--onnx",
        type=str,
        default=str(PROJECT_ROOT / "models" / "yolov8n.onnx"),
        help="Path to YOLOv8n ONNX model",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=str(PROJECT_ROOT / "models" / "yolov8n.rknn"),
